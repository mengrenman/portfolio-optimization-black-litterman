...
a wide daily simple return matrix aligned to the backtest calendar.
...
The mean-variance strategy uses classical Markowitz optimisation driven
entirely by the sample moments of the lookback window.
Let $R$ denote the $T \times n$ matrix of daily simple returns, where $T$ is the
number of trading days in the lookback window and $n$ is the number of assets
in the universe. Each row $\bm{r}_t \in \mathbb{R}^{n}$ is the vector of
simple returns for all $n$ assets on day $t$, so that $R = [\bm{r}_1;\; \bm{r}_2;\;
\ldots;\; \bm{r}_T]$. The sample mean vector and sample covariance matrix are:
\begin{align}
  \hat{\bm{\mu}} &= \frac{1}{T}\sum_{t=1}^{T} \bm{r}_t
    \;\in\; \mathbb{R}^{n}
  \label{eq:mu_hat} \\[4pt]
  \hat{\bm{\Sigma}} &= \frac{1}{T-1}\sum_{t=1}^{T}
    (\bm{r}_t - \hat{\bm{\mu}})(\bm{r}_t - \hat{\bm{\mu}})^{\top}
    \;\in\; \mathbb{R}^{n \times n}
  \label{eq:sigma_hat}
\end{align}
\code{estimate\_mean\_cov()} enforces a minimum of $T \geq 6$ observations,
raising a \code{ValueError} otherwise. This guard prevents degenerate
estimates: with fewer than 6 daily returns the sample mean is dominated by
noise and the covariance matrix has at most rank $T - 1$, which for very small
$T$ can be far below $n$ and render the matrix singular. Any remaining NaN
entries in $\hat{\bm{\Sigma}}$ (arising from tickers that began trading
partway through the window) are replaced with zero.

More generally, a well-conditioned covariance estimate requires $T > n$; when
$T \leq n$ the sample covariance is rank-deficient and the linear system in
Equation~\eqref{eq:mvo_ridge} below becomes ill-conditioned. With the current
configuration (6-month lookback at daily frequency) $T \approx 126$ and
$n \leq 22$, giving a ratio of roughly 6:1 --- comfortably above the bare
minimum and within the range often recommended in practice ($T > 3n$ to $5n$).

\subsection{Long-Only Portfolio Construction}

The unconstrained Markowitz tangency portfolio satisfies:
\begin{equation}
  \bm{w}^{*} = \hat{\bm{\Sigma}}^{-1}\hat{\bm{\mu}}
  \label{eq:mvo_raw}
\end{equation}
In practice, \code{long\_only\_markowitz\_weights()} solves this system with a
small ridge regularisation term added to the diagonal for numerical stability:
\begin{equation}
  (\hat{\bm{\Sigma}} + \delta I)\,\bm{w} = \hat{\bm{\mu}},
  \qquad \delta = 10^{-6}
  \label{eq:mvo_ridge}
\end{equation}
The solution is then projected to a long-only, fully-invested portfolio by
clipping negative weights to zero and renormalising:
\begin{equation}
  w_i = \frac{\max(w_i^{*},\,0)}{\sum_j \max(w_j^{*},\,0)},
  \qquad \sum_i w_i = 1,\quad w_i \geq 0
  \label{eq:mvo_clip}
\end{equation}
If all raw weights are non-positive (degenerate solution), the strategy falls
back to an equal-weight allocation $w_i = 1/n$ and logs a warning.
...
\begin{tabular}{@{}cl@{}}
  ...
  \midrule
  $R$                     & $T \times n$ matrix of daily simple returns \\
  $T$                     & Number of trading days in the lookback window \\
  $n$                     & Number of assets in the universe \\
  $\bm{r}_t$              & $n$-vector of simple returns on day $t$ (row $t$ of $R$) \\
  $\hat{\bm{\mu}}$        & $n$-vector of sample mean returns \\
  $\hat{\bm{\Sigma}}$     & $n \times n$ sample covariance matrix \\
  $\bm{w}^{*}$            & $n$-vector of unconstrained Markowitz weights \\
  $w_i$                   & Weight of asset $i$ after long-only projection \\
  $\delta$                & Ridge regularisation scalar (default: $10^{-6}$) \\
...
\begin{limitbox}
  The sample mean $\hat{\bm{\mu}}$ is notoriously noisy: estimation error in
  expected returns dominates portfolio selection and often produces extreme,
  concentrated allocations. This is the central motivation for the
  Black-Litterman framework, which replaces the sample mean with a
  Bayesian posterior that shrinks towards an equilibrium prior and is therefore
  less sensitive to estimation noise.
\end{limitbox}
...
The Black-Litterman model combines two sources of information --- an
\emph{equilibrium prior} derived from market-clearing weights, and a set of
\emph{investor views} derived from the disclosed portfolio --- to produce a
posterior distribution over expected returns. Both sources share a common
asset covariance matrix $\bm{\Sigma} = \hat{\bm{\Sigma}} \in \mathbb{R}^{n
\times n}$, the same sample covariance estimated in the MVO section
(Equation~\ref{eq:sigma_hat}).

The market-weight vector $\bm{w}_{\text{mkt}} \in \mathbb{R}^{n}$ is
constructed from the disclosed portfolio: the filed dollar weights are
re-indexed to the $n$ valid tickers in the current lookback window (tickers
with all-NaN returns are excluded) and renormalised so that
$\mathbf{1}^{\top}\bm{w}_{\text{mkt}} = 1$. These weights serve as a proxy
for the ``market portfolio'' within the filing's universe --- the allocation
that, under equilibrium assumptions, clears the market.

\subsection{Equilibrium Prior}

The framework begins by computing the \emph{implied equilibrium excess return
vector} by reverse-engineering the market-clearing condition. Given
$\bm{w}_{\text{mkt}}$, the risk-aversion scalar $\lambda$, and the covariance
matrix $\bm{\Sigma}$:
\begin{equation}
  ...
  \label{eq:pi}
\end{equation}
with $\lambda = 2.5$ (default). Intuitively, $\bm{\pi}$ answers the
question: ``What expected returns would make a mean-variance investor
willingly hold $\bm{w}_{\text{mkt}}$?''  These equilibrium returns serve as
the prior mean for the distribution of expected returns.

\subsection{Views from Disclosures}

The disclosed portfolio weights are used to construct a set of investor
\emph{views} about expected asset returns. In this implementation the
view-picking matrix $P$ is set to the $n \times n$ identity matrix:
\begin{equation}
  P = I_n \;\in\; \mathbb{R}^{n \times n}
  \label{eq:P}
\end{equation}
This means there is one \emph{absolute view} per asset --- each view states a
belief about the expected return of a single asset rather than a relative
comparison between assets. The corresponding view return vector
$\bm{q} \in \mathbb{R}^{n}$ is set to the sample mean return vector
$\hat{\bm{\mu}}$ from the lookback window (Equation~\ref{eq:mu_hat}):
\begin{equation}
  \bm{q} = \hat{\bm{\mu}}
  \label{eq:q}
\end{equation}
Since the return matrix $R$ contains daily simple returns and the default
lookback window spans 6~months ($T \approx 126$ trading days), each element
$q_i = \hat{\mu}_i$ is a \emph{mean daily simple return} --- typically on the
order of $10^{-4}$ --- not an annualised or monthly figure.
The uncertainty in these views is modelled as proportional to the prior
covariance, scaled by the confidence parameter $c \in (0, 1)$:
\begin{equation}
  \bm{\Omega} = \frac{1 - c}{c}\;P\,\bm{\Sigma}\,P^{\top}
  \;\in\; \mathbb{R}^{n \times n}
  \label{eq:omega}
\end{equation}
The parameter $c$ (default: $0.65$) controls the relative weight given to
the views versus the equilibrium prior. As $c \to 1$ the view uncertainty
$\bm{\Omega} \to 0$ and the posterior collapses to the sample-mean view; as
$c \to 0$ the uncertainty grows without bound and the posterior collapses to
the equilibrium prior $\bm{\pi}$.
...
\begin{equation}
  ...
  \label{eq:M}
\end{equation}
where $\tau = 0.05$ (default) is a scalar that controls the uncertainty in
the equilibrium prior --- it scales the prior covariance so that
$\tau\bm{\Sigma}$ represents the investor's uncertainty about the true mean
returns (as distinct from $\bm{\Sigma}$ itself, which represents the
uncertainty in realised returns). The posterior mean and covariance are:
\begin{align}
  \bm{\mu}_{\BL} &= M^{-1}\Bigl[
  ...
  \label{eq:SigmaBL}
\end{align}
The posterior mean $\bm{\mu}_{\BL}$ is a precision-weighted blend of the
equilibrium prior $\bm{\pi}$ and the sample-mean views $\bm{q}$: the more
confident the views (higher $c$, smaller $\bm{\Omega}$), the closer
$\bm{\mu}_{\BL}$ sits to $\bm{q}$; the less confident, the closer it sits to
$\bm{\pi}$. The posterior covariance $\bm{\Sigma}_{\BL}$ is always larger
than $\bm{\Sigma}$ alone, reflecting the additional estimation uncertainty
introduced by the Bayesian blending step.

The pair $(\bm{\mu}_{\BL},\,\bm{\Sigma}_{\BL})$ is then passed to the same
long-only solver used in MVO (Equations~\ref{eq:mvo_ridge}--\ref{eq:mvo_clip})
with the full-investment constraint $\mathbf{1}^{\top}\bm{w} = 1$,
$\bm{w} \geq 0$. If the solver returns all non-positive weights the strategy
falls back to equal-weight allocation and logs a warning.
...
\begin{tabular}{@{}cl@{}}
  ...
  \textbf{Symbol} & \textbf{Meaning} \\
  \midrule
  $\bm{\Sigma}$            & $n \times n$ sample covariance matrix (same as $\hat{\bm{\Sigma}}$ from MVO) \\
  $\bm{w}_{\text{mkt}}$    & $n$-vector of disclosed portfolio weights (renormalised to valid tickers) \\
  $\lambda$                & Risk-aversion scalar (default: $2.5$) \\
  $\bm{\pi}$               & $n$-vector of implied equilibrium excess returns \\
  $P$                      & $n \times n$ view-picking matrix (identity: one absolute view per asset) \\
  $\bm{q}$                 & $n$-vector of view returns (set to sample mean $\hat{\bm{\mu}}$) \\
  $c$                      & View confidence scalar (default: $0.65$; swept in Notebook~03) \\
  $\bm{\Omega}$            & $n \times n$ view uncertainty matrix \\
  $\tau$                   & Prior uncertainty scalar (default: $0.05$) \\
  $M$                      & $n \times n$ posterior precision matrix \\
  $\bm{\mu}_{\BL}$         & $n$-vector of BL posterior expected returns \\
  $\bm{\Sigma}_{\BL}$      & $n \times n$ BL posterior covariance matrix \\
  \bottomrule
\end{tabular}
\end{center}
